...
export const maxZoomDelta = ({ top, left, bottom, right }) => {
  ...
  let latitudeOffset = 0,
    longitudeOffset = 0;
  if (isWithin(top, [0, 1]))
    latitudeOffset -= latitudeDelta / 2 - top * latitudeDelta;
  else if (isWithin(bottom, [0, 1]))
    latitudeOffset += latitudeDelta / 2 - bottom * latitudeDelta;
  if (isWithin(left, [0, 1]))
    longitudeOffset -= longitudeDelta / 2 - left * longitudeDelta;
  else if (isWithin(right, [0, 1]))
    longitudeOffset += longitudeDelta / 2 - right * longitudeDelta;
  return {
    longitudeOffset,
    latitudeOffset,
    longitudeDelta,
...
const isWithin = (number, range) =>
  number && number <= range[1] && number >= range[0];
